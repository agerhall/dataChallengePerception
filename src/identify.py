...
#    data = data.astype(float)  # ask for non-rounding arithmetic
    ...
    #data = np.ma.concatenate([data[:, 0], data[:, ]], axis=1)
    return data
...
class KalmanJoint:
    def __init__(self, var):
        self.kf = pk.KalmanFilter(
            ...
            observation_covariance=0*np.array([[var[0],0.0],[0.0,var[1]]]), #np.array([[0.0,0.0],[0.0,0.0]]),
    ...
    def likelihood(self, observation):
        ...
        if np.ma.is_masked(x) or np.ma.is_masked(y):
            mean, var = pk.standard._filter_predict(
                self.kf.transition_matrices, self.kf.transition_covariance, self.kf.transition_offsets,
                self.kf.initial_state_mean, self.kf.initial_state_covariance)
            n = mean.shape[0]
            v = np.linalg.det(var)
            return -n*0.5*(1 + np.log(2*np.pi*v))
        #print(x, y)
        assert x != -1 and y != -1

        # hack
        self.kf.initial_state_covariance += self.kf.transition_covariance
        ret = self.kf.loglikelihood([observation])
        self.kf.initial_state_covariance -= self.kf.transition_covariance
        return ret
...
def train(data):
    return np.mean([np.ma.diff(i, axis=0).var(axis=0) for i in by_person(data)], axis=0)
...
def head_mean(obs):
    o = np.ma.mask_equal(obs.values, -1)
    f = lambda i: [o[i], o[i+1]]
    v = np.ma.array(map(f, (3, 5, -2, -4, -6, -8)))
